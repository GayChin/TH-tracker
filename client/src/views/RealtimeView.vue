...
socket.on("realtime-temperature-data", (data) => {
    if (
        !areArraysOfArraysOfObjectsEqual(historicalTemperatureData.value, data)
    )
        historicalTemperatureData.value = data
});

socket.on("realtime-humidity-data", (data) => {
    if (
        !areArraysOfArraysOfObjectsEqual(historicalHumidityData.value, data)
    )
        historicalHumidityData.value = data
});


// Helper function to deeply compare two arrays of arrays of objects
function areArraysOfArraysOfObjectsEqual(arr1, arr2) {
...
const realtimeInterval = setInterval(() => {
    socket.emit("request-temperature-realtime-data", { ...queryOptions.value, field: 'temperature' })
    socket.emit("request-humidity-realtime-data", { ...queryOptions.value, field: 'humidity' })
}, 2000)
