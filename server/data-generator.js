...
export const generateData = () => {
  ...
  for (let i = 0; i < SITE_DATA.locations.length; i += 1) {
    ...
    const locationDevices = SITE_DATA.locations[i].devices;
    for (let j = 0; j < locationDevices.length; j += 1) {
      const deviceId = locationDevices[j];
    ...
    // console.log(`Published data for ${deviceId} @${timestamp}`);
    }
  }
};
